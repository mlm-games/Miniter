...
fn hue_shift_rgba(src: &[u8], degrees: f32) -> Vec<u8> {
    ...
    for px in out.chunks_exact_mut(4) {
        ...
        let rn = 0.299f32 + 0.701 * cos_a + 0.168 * sin_a;
        let gn = 0.587f32 - 0.587 * cos_a + 0.330 * sin_a;
        let bn = 0.114f32 - 0.114 * cos_a - 0.497 * sin_a;

        let rn2 = 0.299f32 - 0.299 * cos_a - 0.328 * sin_a;
        let gn2 = 0.587f32 + 0.413 * cos_a + 0.035 * sin_a;
        let bn2 = 0.114f32 - 0.114 * cos_a + 0.292 * sin_a;

        let rn3 = 0.299f32 - 0.300 * cos_a + 1.250 * sin_a;
        let gn3 = 0.587f32 - 0.588 * cos_a - 1.050 * sin_a;
        let bn3 = 0.114f32 + 0.886 * cos_a - 0.203 * sin_a;

        let nr = (rn * r + gn * g + bn * b).clamp(0.0, 1.0);
        let ng = (rn2 * r + gn2 * g + bn2 * b).clamp(0.0, 1.0);
        let nb = (rn3 * r + gn3 * g + bn3 * b).clamp(0.0, 1.0);

        px[0] = (nr * 255.0).round().clamp(0.0, 255.0) as u8;
        px[1] = (ng * 255.0).round().clamp(0.0, 255.0) as u8;
        px[2] = (nb * 255.0).round().clamp(0.0, 255.0) as u8;
    }
...
fn flip_rgba(src: &[u8], width: usize, height: usize, horizontal: bool, vertical: bool) -> Vec<u8> {
    ...
    for y in 0..height {
        ...
        for x in 0..width {
            ...
            let si = (src_y * src_stride) + src_x * 4;
            out[di..di + 4].copy_from_slice(&src[si..si + 4]);
        }
    }
